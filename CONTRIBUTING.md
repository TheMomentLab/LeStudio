...
- `packages/lerobot-checkup` — hardware layer library (device discovery, udev/type/path policy, motor and calibration bridges). No server or CLI yet.
- `packages/lestudio` — the workbench: FastAPI backend under `src/lestudio/` and the React frontend under `frontend/`. Depends on `lerobot-checkup`.
...
4. `packages/lerobot-checkup/src/lerobot_checkup/device_registry.py`
5. `packages/lerobot-checkup/src/lerobot_checkup/motor_monitor_bridge.py`
...
- `packages/lerobot-checkup/src/lerobot_checkup/calibrate_bridge.py` — Uses `importlib.import_module()` for dynamic robot-type resolution
...
- `packages/lerobot-checkup/src/lerobot_checkup/motor_setup_bridge.py` — Spawns `lerobot_setup_motors` as a subprocess
...
python3 -m mypy packages/lerobot-checkup/src/lerobot_checkup packages/lestudio/src/lestudio --ignore-missing-imports
python3 -m compileall -q packages/lerobot-checkup/src/lerobot_checkup packages/lestudio/src/lestudio
make test
```
