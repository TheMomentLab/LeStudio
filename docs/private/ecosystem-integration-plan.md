...
src/lestudio/
├── cli.py                  # 진입점
├── server.py               # FastAPI 서버 (핵심 — ~2200줄)
├── device_registry.py      # LeRobot 3-Registry 통합 탐색 (~540줄)
...
└── static/                 # Vite 빌드 결과물 (React 19 + TypeScript)

frontend/                    # React 프론트엔드 소스
├── src/
│   ├── App.tsx              # 루트 컴포넌트 (탭 라우팅, 테마)
│   ├── store/index.ts       # Zustand 전역 스토어
│   ├── tabs/                # 9개 탭 컴포넌트 (Status~Eval)
│   ├── components/          # 공유 컴포넌트 (layout/, shared/)
│   └── hooks/               # 커스텀 훅 (useConfig, useProcess 등)
└── package.json             # React 19, Zustand 5, Vite 7
```
...
#### Frontend (React 컴포넌트 — `frontend/src/tabs/`)
| 위치 | 하드코딩 내용 | 영향 |
|------|-------------|------|
| `TeleopTab.tsx` | 포트 기본값, role 매핑 (follower/leader) | SO-101 이외 로봇 지원 불가 |
| `RecordTab.tsx` | 동일한 포트/role 가정 | 동일 |
| `CalibrateTab.tsx` | 캘리브레이션 대상 role 고정 | 동일 |
| `MotorSetupTab.tsx` | 모터 ID 기본값, 모터 모델 가정 | Dynamixel/Damiao 모터 미지원 |
| `DeviceSetupTab.tsx` | 시리얼 포트 패턴, validation 규칙 | 네트워크 로봇 미지원 |
 ...
 **안전 제한 설정 UI** (roadmap 3.5단계): 관절 범위 제한, 최대 속도, 비상 정지 등을 `RobotProcessorPipeline`의 safety limits로 구현 가능.
...
### 4.3 Frontend 재설계 (React 19 + TypeScript 기반)
...
| 프론트엔드 로봇 선택기 | `frontend/src/components/` | Robot + Teleoperator 분리 선택 드롭다운 + 로봇 정보 카드 |
...
| 작업 | 파일 | 설명 |
|------|------|------|
| 네트워크 설정 패널 | `frontend/src/components/` | IP/port 입력, 연결 테스트 버튼 |
| 모바일 베이스 제어 패널 | `frontend/src/components/` | 방향키 UI, 속도 제어, 키 매핑 |
| 양팔 설정 패널 | `frontend/src/components/` | Left/Right 독립 구성 |
| Capability 기반 패널 토글 | `frontend/src/tabs/` | capabilities 응답 기반 조건부 렌더링 |
| 카메라 타입별 설정 폼 | `frontend/src/components/` | opencv/zmq/realsense 각각의 설정 UI |
| Teleoperator 선택기 UI | `frontend/src/components/` | 호환 텔레옵 드롭다운 + 설정 폼 |
| CAN bus 설정 UI | `frontend/src/components/` | CAN 인터페이스 설정, setup-can 실행 |
| Processor 설정 노출 (선택) | `frontend/src/components/` | record 시 processor pipeline 옵션 |
...
| 프론트엔드 복잡도 증가 | 컴포넌트 간 상태 동기화 복잡도 | React 19 + Zustand 전역 스토어로 전환 완료, 컴포넌트 분리 유지 |
| 플러그인 패키지 품질 불균일 | 잘못된 config schema, 누락 필드 | 방어적 파싱, 에러 시 해당 타입만 스킵 |

### 6.2 LeRobot Bridge 격리 원칙

현재 LeRobot 직접 결합은 4개 파일에 격리되어 있다:
 `teleop_bridge.py`
 `record_bridge.py`
 `camera_patch.py`
 `device_registry.py`
...
- [x] **Q4**: 프론트엔드 프레임워크 전환 시점은?
  - **→ 완료: React 19 + TypeScript + Vite 7 + Zustand으로 전환 (2단계~2.5단계에서 실행)**
  - 근거: Vanilla JS 18모듈 구조가 한계에 도달하여 React 19 + TypeScript로 전면 전환함. Zustand 단일 스토어로 전역 상태를 관리하고, Vite 7 빌드 결과물을 `src/lestudio/static/`에 배포. 9개 탭 컴포넌트 + 공유 컴포넌트 + 커스텀 훅 구조로 재설계 완료.
