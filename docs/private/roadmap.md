...
프론트엔드: React 19 + TypeScript + Vite 7 + Zustand (9개 탭, 32 소스 파일)
...
| 2.5단계 (UI/UX 안정화 + React 전환) | **완료** | 모바일 헤더/상태 피드백/접근성 + Vanilla JS → React 19 + TS 전환 완료 |
| 3.0단계 (안정화 게이트) | **완료** | 보안 경로 차단, 경계 정합성, 테스트/CI 최소선 확보 |
| 3단계 (OSS 준비) | **다음** | 3.0 완료 후 착수 |
| 3.1단계 (리팩토링) | **다음** | 코드 품질 개선 — [`refactoring-plan.md`](refactoring-plan.md) 참조 |
...
- [x] 프론트엔드 React 19 + TypeScript + Zustand 전환 (Vanilla JS에서 마이그레이션 완료)

**입력 문서**: `docs/uiux-audit.md` → 삭제됨, 통합 리뷰는 `docs/private/archive/uiux-review-2026-02-24.md`에 보관
...
- `lerobot.*` 결합 경계(4접점 원칙)를 만족한다.

**실행 기준 문서**: [`quality-improvement-plan.md`](quality-improvement-plan.md)
...
- [ ] Public Docs Site — LeRobot 접점 확대용 문서 페이지 (MkDocs 권장) ([`docs-site-plan.md`](docs-site-plan.md))


**전략 문서**: [`strategy-competitive-analysis.md`](strategy-competitive-analysis.md) — 경쟁 환경 분석, 포지셔닝, 런칭 전략
---
...
- [ ] 언어 선택 / i18n 점진 도입 — 전면 번역은 나중에, locale 상태/공통 영역부터 정리 ([`language-selection-i18n.md`](language-selection-i18n.md))
...
**상세 설계 문서**: [`docs/private/ecosystem-integration-plan.md`](ecosystem-integration-plan.md) (v2 — LeRobot 공식 문서 분석 반영)
**핵심 변경**:
...
**구현 Phase**:
  Phase 0: 추상화 레이어 삽입 (DeviceRegistry 3-Registry + 플러그인 발견) — **백엔드 구현 완료** (`device_registry.py` 540줄)
...
**아키텍처 참고**: LeRobot 직접 결합은 bridge 3파일 + `device_registry.py`(총 4접점)로 격리한다.
---
