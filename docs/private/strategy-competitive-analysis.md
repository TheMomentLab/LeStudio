...
- **독립 GUI 시도 5건** — leLab(21⭐), Any4LeRobotGUI(5⭐), ratsbane/robot-web(4⭐), bambot, lerobotlab.com — 커뮤니티 내 GUI 수요 반복 확인
...
| 디버깅/진단 도구 없음 | SHM 카메라, udev, USB 대역폭 등 **6개 하드웨어 진단 도구** + 에러 번역, Loss 차트, 체크포인트 브라우저 등 **7개 실험 관리 기능** |
...
| **USB 대역폭 모니터링** | 피드별 실시간 fps/MB·s, USB 버스 사용률 바 — 대역폭 고갈 사전 감지 | `server.py` + `tabs/DeviceSetupTab.tsx` |
...
| 기능 | 설명 |
|---|---|
| **에러 번역 레이어** | CLI stderr 패턴 5개 → 사용자 친화적 가이드 메시지 변환 |
...
1. **LeRobot 네이티브**: CLI 래핑 구조 → LeRobot이 지원하는 모든 로봇을 자동 지원 (Phase 0 백엔드 완료, 14+ 타입)
2. **진단/운영 도구**: SHM 카메라, udev, USB 대역폭, CUDA preflight 등 6개 하드웨어 진단 도구 + 7개 실험 관리 기능 — 경쟁자 0개가 보유
3. **완전 무료 + 로컬 우선**: 학습 무제한, 데이터 로컬, 유료 벽 없음

### 5.3 AI 속도 활용 전략

LeStudio는 AI를 활용하여 초기 프로토타입을 3일 만에 구현하고, 이후 React 전환까지 완료하여 현재 ~5,900줄/60 API 규모이다.
이 속도는 경쟁 전략에 구조적 이점을 제공한다:
- **Phase 0 (SO-101 하드코딩 제거)** → 백엔드 완료 (`device_registry.py` 540줄), Phase 1 (GenericCommandBuilder, ConnectionAdapter) 미착수
...
| 항목 | 근거 |
|---|---|
| Phase 0 완료 (다중 로봇 기반) | Phase 0 백엔드 완료 (`device_registry.py` 540줄). Phase 1 (GenericCommandBuilder, ConnectionAdapter)은 공개 후 진행. SO-101 전용이라도 기본 타입 매핑은 동작하므로 공개 가능 |
...
- 기술 설계: [`../design-phase-0-1.md`](../design-phase-0-1.md), [`../design-phase-1.md`](../design-phase-1.md)
