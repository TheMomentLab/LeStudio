...
- [x] capability 기반 정책/보호 구조로 정리 — `capabilities.py` Capability enum + route 등록 + `_auth.py` prefix 매칭 교체 완료
...
- [x] route orchestration을 service 계층으로 분리 — D1(process), D2(dataset), D3(training) 전부 완료
- [x] 정합성 회귀 테스트 보강 — path_policy/calibration_validator 단위 테스트, service delegation integrity 테스트, process_manager EventBuffer 테스트 수정 (171 passed)
