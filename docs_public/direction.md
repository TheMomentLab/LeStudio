...
4. **Answer where the pain is** — point LeRobot port / motor / camera issues at the tool. *Started 2026-09-08. The troubleshooting guide, the package README and the bug-report template now map LeRobot's own error strings (`Motor ... was not found`, `There is no status packet`, `Read failed due to communication error`, `No space left on device`, ...) to the command that answers each one. A survey of the 356 open LeRobot issues found 52 hardware-setup ones (15%); the target list with draft replies is in the outreach plan. Replies go out only after the first PyPI release, by hand, on issues the tool actually answers.*
