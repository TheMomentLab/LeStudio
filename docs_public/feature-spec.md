...
> **방향 (2026-09).** LeLab이 LeRobot 공식 GUI가 되면서 LeStudio의 무게 중심은 **하드웨어 설정·진단**(장치 매핑, 모터 설정 위저드, 모터 모니터, 캘리브레이션 검증, 카메라 설정)으로 옮겨졌습니다. 이 층은 독립 패키지(가칭 `lerobot-checkup`)로 분리될 예정이며, 아래 §4.2와 §4.6의 모터 모니터 항목이 그 대상입니다. 나머지 섹션은 워크벤치입니다. 자세한 내용은 `direction.md`.
