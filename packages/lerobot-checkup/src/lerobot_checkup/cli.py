...
(``--json``) so the answer can be pasted into a LeRobot issue verbatim:
    ...
    lerobot-checkup cameras               V4L2 cameras and their USB bus
    lerobot-checkup motors --port PORT    ping Feetech motors, read pos/load/current
    lerobot-checkup calibration [FILE..]  validate calibration files
    lerobot-checkup udev status|install   stable /dev symlinks
    lerobot-checkup report                everything above, as Markdown
    lerobot-checkup serve                 the web UI (also the default with no arguments)
...
from lerobot_checkup import __version__, calibration_validator, device_helpers, path_policy  # noqa: E402
...
def read_motors(port: str, motor_ids: list[int], model: str = DEFAULT_MOTOR_MODEL, *, samples: int = 5) -> dict:
    ...
    """
    from lerobot_checkup.motor_monitor_bridge import MotorMonitorBridge
...
def render_report(report: dict[str, Any]) -> str:
    ...
    lines.append(f"- lerobot-checkup: {system['lerobot_checkup']}")
...
def cmd_serve(args: argparse.Namespace) -> int:
    from lerobot_checkup import serve as srv
    from lerobot_checkup._auth import generate_token
    from lerobot_checkup.server import create_app
    ...
    srv.run_uvicorn(
        ...
        port=args.port,
        reload=args.reload,
        reload_factory="lerobot_checkup.server:create_app_from_env",
        reload_env={
            "_LEROBOT_CHECKUP_LEROBOT_SRC": str(lerobot_src),
            "_LEROBOT_CHECKUP_CONFIG_DIR": str(config_dir),
            "_LEROBOT_CHECKUP_RULES_PATH": str(args.rules_path),
            "_LEROBOT_CHECKUP_TOKEN": token,
        },
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"lerobot-checkup {__version__}")
