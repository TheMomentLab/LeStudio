...
``create_app`` is the standalone ``lerobot-checkup serve`` app. ``make_state``
...
from lerobot_checkup._auth import TokenAuthMiddleware, generate_token
from lerobot_checkup._cors import _resolve_cors_settings
from lerobot_checkup._device_watcher import DeviceWatcher
from lerobot_checkup._logging import configure_logging
from lerobot_checkup._streaming import unlock_cameras
from lerobot_checkup.process_manager import ProcessManager
from lerobot_checkup.routes._state import AppState
...
def hardware_routers(state: AppState) -> list[APIRouter]:
    from lerobot_checkup.routes import config, devices, motor, process, streaming, udev
...
def create_app(
    ...
    """The standalone lerobot-checkup web app."""
    state = make_state(config_dir, rules_path, lerobot_src=lerobot_src, state_cls=AppState)
    return build_app(state, title="lerobot-checkup", static_dir=STATIC_DIR, session_token=session_token)


def create_app_from_env() -> FastAPI:
    """App factory for ``uvicorn --reload`` mode (environment set by the CLI)."""
    return create_app(
        lerobot_src=Path(os.environ["_LEROBOT_CHECKUP_LEROBOT_SRC"]),
        config_dir=Path(os.environ["_LEROBOT_CHECKUP_CONFIG_DIR"]),
        rules_path=Path(os.environ["_LEROBOT_CHECKUP_RULES_PATH"]),
        session_token=os.environ.get("_LEROBOT_CHECKUP_TOKEN") or generate_token(),
    )
