...
from lerobot_checkup.server import create_app as create_checkup_app
...
def test_checkup_app_serves_hardware_routes_only(tmp_path: Path):
    app = _make(tmp_path, create_checkup_app)
    paths = _paths(app)
    assert HARDWARE <= paths
    assert not (WORKFLOW & paths)
    assert app.title == "lerobot-checkup"
