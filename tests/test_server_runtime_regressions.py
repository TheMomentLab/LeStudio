...
def test_api_proc_stop_train_stops_train_and_installer(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("lerobot_checkup.process_manager.ProcessManager.stop", fake_stop)
    monkeypatch.setattr("lerobot_checkup.routes.process.unlock_cameras", fake_unlock)
...
def test_api_record_start_stops_streamers_and_injects_camera_settings(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("lerobot_checkup.process_manager.ProcessManager.is_running", lambda self, name: False)
    monkeypatch.setattr("lerobot_checkup.process_manager.ProcessManager.conflicting_processes", lambda self, name: [])
...
def test_api_preflight_uses_bimanual_defaults_for_non_single_mode(monkeypatch, tmp_path: Path):
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    # Preflight validates single-arm files in lestudio and bimanual pairs through lerobot_checkup.
    for module in ("lestudio.services.process_service", "lerobot_checkup.services.process_service"):
        monkeypatch.setattr(
...
def test_api_type_policy_catalog_still_returns_static_policy_when_lerobot_unavailable(monkeypatch, tmp_path: Path):
    monkeypatch.setattr("lerobot_checkup.device_registry._LEROBOT_AVAILABLE", False)
...
def test_api_calibrate_file_supports_bimanual_shared_profile_id(monkeypatch, tmp_path: Path):
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setattr(
        "lerobot_checkup.services.process_service.validate_calibration_file",
        lambda path, **kwargs: types.SimpleNamespace(errors=[], warnings=[]),
    )
...
def test_snapshot_camera_returns_streamer_frame(monkeypatch, tmp_path: Path):
    monkeypatch.setattr("lerobot_checkup.process_manager.ProcessManager.is_running", lambda self, name: False)
    monkeypatch.setattr("lerobot_checkup.routes.streaming.snapshot_get_frame", lambda video_path, config_path: b"jpeg-bytes")
...
def test_snapshot_camera_returns_503_when_frame_unavailable(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("lerobot_checkup.process_manager.ProcessManager.is_running", lambda self, name: False)
    monkeypatch.setattr("lerobot_checkup.routes.streaming.snapshot_get_frame", lambda video_path, config_path: None)
    monkeypatch.setattr("lerobot_checkup.routes.streaming.asyncio.sleep", _fast_sleep)
...
def test_train_preflight_cache_is_used_and_invalidated(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr("lerobot_checkup.process_manager.ProcessManager.is_running", lambda self, name: False)
    monkeypatch.setattr("lerobot_checkup.process_manager.ProcessManager.start", lambda self, name, args: True)
